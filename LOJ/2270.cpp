...
void solve() {
	...
	rep(x, 1, n) {
		...
		dfs(1);
		for(auto v : R[x]) link(v, outid[d[v]], c[v]), link(v, toid[d[v]], c[v] + dep[d[v]] * 2);
		for(auto v : G[x]) link(toid[d[v]], v, c[v]), link(outid[d[v]], v, c[v] + dep[d[v]] * 2);
		clr(1);
