...
Z T(int x) { return Z(n - x) * Z(m - x) * Z(l - x); }

void solve() {
	n = in, m = in, l = in, K = in; t = min({ n, m, l }); Z ans = 0, pt = 1;
	Z ret = 1; rep(i, 1, t) ret = ret * (T(0) - T(i)); ret = qp(ret, mod - 2);
	per(i, t, 1) val[i] = ret, ret *= (T(0) - T(i));
	rep(i, 1, t) {
		pt *= n - i + 1, pt *= m - i + 1, pt *= l - i + 1;
		if(i >= K) {
			ans += ((i - K) & 1 ? -1 : 1) * C(i, K) * pt * val[i];
		}
	} printf("%d\n", ans.val());
}
