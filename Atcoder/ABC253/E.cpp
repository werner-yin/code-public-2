...
#endif
	n = in, m = in, K = in;
	if(K == 0) return cout << qp(Z(m), n).val() << endl, 0;
	int o = 0; rep(i, 1, m) f[o][i] = 1;
	rep(i, 2, n) {
		rep(j, 1, m) s[j] = s[j - 1] + f[o][j];
		rep(j, 1, m) {
			f[o ^ 1][j] = s[m] - s[min(j + K - 1, m)] + s[max(j - K + 1, 1) - 1];
		} o ^= 1;
		//rep(j, 1, m) cerr << f[o][j].val() << " "; cerr << endl;
	}
	Z ans = 0; rep(i, 1, m) ans += f[o][i]; cout << ans.val() << endl;
	return 0;
}
