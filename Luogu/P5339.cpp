...
Z solve(int x) {
	rep(i, 0, n - 4 * x) f[i] = 0; f[0] = 1;
	for(int v : { a - x, b - x, c - x, d - x }) {
		rep(i, 0, n - 4 * x) g[i] = 0;
		rep(i, 0, n - 4 * x) if(f[i].val())
			rep(t, 0, v)
				g[i + t] += f[i] * ifac[t];
		rep(i, 0, n - 4 * x) f[i] = g[i];
	}
	return f[n - 4 * x] * fac[n - 4 * x];
}
...
#endif
	n = in, a = in, b = in, c = in, d = in; init(n);
	rep(x, 0, min({ n / 4, a, b, c, d }))
		ans += Z(x & 1 ? -1 : 1) * C(n - 3 * x, x) * solve(x);
	cout << ans.val() << endl;
	return 0;
}
