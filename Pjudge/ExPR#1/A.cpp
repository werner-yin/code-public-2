...
unordered_map < int, Z > f;

Z solve(int m) {
	if(m == 1) return vt;
	if(f.count(m)) return f[m]; Z ret = 0;
	//if(n > m) ret += n - m;
	for(int l = 2, r; l <= m && l <= n; l = r + 1) {
		r = min(m / (m / l), n);
		ret += (r - l + 1) * solve(m / l);
	} ret *= iv; ret += vt; return f[m] = ret;
}
...
#endif
	n = in, m = in; iv = Z(n - 1).inv(); vt = n * iv; cout << solve(m).val() << endl;
	return 0;
}
