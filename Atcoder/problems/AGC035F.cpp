...
#endif
	n = in, m = in; init(max(n, m));
	pwn[0] = 1; rep(i, 1, m) pwn[i] = pwn[i - 1] * (n + 1);
	pwm[0] = 1; rep(i, 1, n) pwm[i] = pwm[i - 1] * (m + 1);
	rep(x, 0, min(n, m)) {
		Z ret = C(n, x) * C(m, x) * fac[x] * pwn[m - x] * pwm[n - x];
		if(x & 1) ans -= ret; else ans += ret;
	} cout << ans.val() << endl;
	return 0;
}
