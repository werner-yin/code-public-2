...
int main() {
	...
	rep(i, 0, P - 1) id[i] = -1;
	int p = 1; while(id[p] == -1) id[p] = tot++, ind[id[p]] = p, p = 1ll * p * T % P;
	rep(i, 1, P - 1) vcnt[id[i]] = cnt[i];
	cerr << tot << endl;
	ll ans = 0;
	poly t(P - 1, 0);
	//cerr << ind[0] << " " << ind[1] << " " << ind[P - 2] << endl;
	rep(i, 0, P - 2) if(vcnt[i] > 60) {
		//rep(j, 0, P - 2) ans += 1ll * vcnt[i] * vcnt[j] * ind[(i + j) % (P - 1)];
		ok[i] = true;
	} else t[i] += vcnt[i];
	rep(i, 0, P - 2)
		if(ok[i])
			rep(j, 0, P - 2) ans += 1ll * vcnt[i] * vcnt[j] * ind[(i + j) % (P - 1)] * (!ok[j] ? 2 : 1);
	t = t * t;
	rep(i, 0, (int)t.size() - 1) {
		ans += 1ll * t[i].val() * ind[i % (P - 1)];
	} rep(i, 1, n) ans -= 1ll * a[i] * a[i] % P; cout << ans / 2 << endl;
	return 0;
}
