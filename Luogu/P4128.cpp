...
template < typename T > T qp(T x, ll t) { t %= mod - 1; T res = 1; for(; t; t >>= 1, x *= x) if(t & 1) res *= x; return res; }
...
void dfs(int lef) {
	if(lef == 0) {
		ll tot = 0;
		rep(i, 1, top) tot += stk[i] >> 1;
		rep(i, 1, top) rep(j, i + 1, top) tot += __gcd(stk[i], stk[j]);
		Z ways = 1; rep(i, 1, top) ways *= fac[stk[i] - 1] * ifac[stk[i]];
		rep(l, 1, top) {
			int r = l; while(stk[l] == stk[r] && r <= top) r++; r--;
			ways *= ifac[r - l + 1]; l = r;
		}
		ans += ways * qp(Z(m), tot); return;
	}
	rep(i, max(1, stk[top]), lef) stk[++top] = i, dfs(lef - i), top--;
}
...
#endif
	n = in, m = in, mod = in; init(n);
	dfs(n); cout << ans.val() << endl;
	return 0;
}
