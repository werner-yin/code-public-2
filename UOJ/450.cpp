...
#endif
	n = in, k = in, d = in; init(k);
	if(d == 1) printf("%d\n", qp(Z(k), n).val());
	else if(d == 2) {
		Z ans = 0; Z w2 = qp(G, (mod - 1) / 2); //cerr << (w2 * w2).val() << endl; assert((w2 * w2).val() == 1);
		rep(i, 0, k) ans += C(k, i) * qp(k * w2 - i * w2 + i, n);
		ans /= qp(Z(d), k);
		cout << ans.val() << endl;
	} else {
		Z ans = 0; Z w3 = qp(G, (mod - 1) / 3);
		rep(i, 0, k) rep(j, 0, k - i)
			ans += C(k, i) * C(k - i, j) * qp((k - i - j) * w3 * w3 + j * w3 + i, n);
		ans /= qp(Z(d), k);
		cout << ans.val() << endl;
	}
	return 0;
